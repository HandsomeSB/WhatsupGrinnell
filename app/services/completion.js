...
tc.addTool(
  "getEventsFromDate",
  "Gets events from a given date range at Grinnell, IA",
  {
    type: "object",
    properties: {
      startDate: {
...
export async function generateCompletionWithTools(
  ...
  try {
    ...
    const messages = [
      {
        role: "system",
        content: `You are WhatsupGrinnell, a large language model trained by OpenAI.
    ...
    // If a JSON schema was provided, ensure the response is valid JSON
    if (responseFormat && responseFormat.type === "json_schema") {
      try {
        // Try to parse as JSON, if it fails, wrap the response in a proper JSON structure
        JSON.parse(result);
      } catch (e) {
          ...
          events: [
            {
              ...
              description:
                "The search results could not be processed. Please try again with different search terms.",
            },
          ],
        });
      }
